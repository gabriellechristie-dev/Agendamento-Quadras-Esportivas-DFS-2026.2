...
export const cadastrarJogador = async (request, response) => {
  try {
    ...
    const novoJogador = await jogadorService.cadastrarJogador({ nomeCompleto, email, telefone, senha });
...
export const atualizarJogador = async (request, response) => {
  try {
    ...
    const jogadorAtualizado = await jogadorService.atualizarJogador(id, { nomeCompleto, email, telefone });

    console.log(`Jogador updated com sucesso: ${jogadorAtualizado.nomeCompleto}`);
